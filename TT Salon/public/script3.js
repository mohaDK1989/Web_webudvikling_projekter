...
var thisweekOfCurrentMonth = Date.prototype.getWeek()+2
$(document).ready(function () {
    ...
    $("#uge").text("Uge " + thisweekOfCurrentMonth)
...
// giv atrribut id til alle td*************************
    for(var i = 0;i<arrUge.length;i++){
        ...
        for(var j = 0;j<arrUge[i].length;j++){
            ...
            $(temparr[j]).attr("uge",thisweekOfCurrentMonth)
            var id = thisweekOfCurrentMonth+""+$(temparr[j]).attr("name")+""+$(temparr[j]).text()
    ...
    $(".dage").on("click",function (data) {
        ...
        if( $(data.target).css("background-color").localeCompare(strcolor)!=0  ){
            ...
            $(data.target).attr("uge",thisweekOfCurrentMonth)
    ...
    $("#sub").on("click",function () {
        ...
        var tidid =thisweekOfCurrentMonth+""+dag+""+tid

        var tid={
            navn: navn,
            tlfNr: tel,
            uge:thisweekOfCurrentMonth,
            dag: dag,
            tid:tid,
...
// fra databasen
    ...
    $.getJSON("/tider",function (data) {
        ...
        for(var j=0;j<data.length;j++){
            ...
            if(data[j].uge==thisweekOfCurrentMonth){
                if(data[j]!=null){
